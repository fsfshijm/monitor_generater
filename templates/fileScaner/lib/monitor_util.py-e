...
#MU.registerConnection('monitor', host="xxxxxxxx-200.xxxxxxxx-inc.cn", port=3306, user='qa', passwd='dmqa714monitor', db='monitor', pool_size=8)
...
def insert_alarm_data(sender, object, content, alarm_email, dt, hr):
	...
	try:
		...
		sql = insert_sql % (str(sender), str(content), str(alarm_email), cur_row[0], dt, hr)
		MU.execute('monitor', sql)
	except Exception, e:
		print e


def insert_reports_data(content, object, dt, hr, summed=False):
	"""
	...
	try:
		...
		record_sql = """SELECT content_map from reports where obj_id = %s and dt = '%s' and hr=%s;"""
		obj_id_sql = """SELECT obj_id from objects where obj_name = '%s';"""
		cur_row = MU.fetchone('monitor', obj_id_sql % (object))
		if cur_row == None:
			print 'object is no exit, please create it first!'
		...
		if cur_record != None:
			if summed :
				old_data = eval(cur_record[0])
				for key in new_content:
					new_content[key] += old_data.get(key, 0)
			MU.execute('monitor', update_sql % (str(new_content), cur_row[0], dt, hr))	
		else:
			sql = insert_sql % (str(new_content), str(cur_row[0]), dt, hr)
			MU.execute('monitor', sql)

		update_object_status(object)
...
def update_object_status(object):
        objects_sql = """SELECT obj_id, type, status from objects where obj_name='%s';"""
        cur_row = MU.fetchone('monitor', objects_sql % object)
        cur_time = datetime.now()
        exist_alarms_sql = """select count(*) from alarm_stat where obj_id = %s and type = 0""" #未处理的报警
        exist_reports_sql = """select count(*) from reports where ((dt = '%s' and hr >= %s) or (dt > '%s') ) and obj_id = %s"""
        update_obj_status_sql = """update objects set status='%s' where obj_id = %s"""

        obj_status = 'Normal' #'Normal','UnWached','Warning','Error'
        if int(cur_row[1]) == 1: #小时级
                pre_time = cur_time - timedelta(hours=3) #看前三个小时是否有报警
        elif int(cur_row[1]) == 2:  #2 天级
                pre_time = cur_time - timedelta(days=3) #看前三天是否有报警
        else:   #没有数据的
                obj_status = 'UnWached'
                if obj_status != str(cur_row[2]):
                        MU.execute('monitor',update_obj_status_sql % (obj_status, str(cur_row[0])))
                return
        pre_dt = pre_time.strftime('%Y-%m-%d')
        pre_hr = pre_time.strftime('%H')
        reports_row = MU.fetchone('monitor',exist_reports_sql % (pre_dt, pre_hr, pre_dt, str(cur_row[0])))
        alarms_row = MU.fetchone('monitor',exist_alarms_sql % (str(cur_row[0])))
        if int(reports_row[0]) == 0:
                obj_status = 'Warning'
        if int(alarms_row[0]) > 0:
                obj_status = 'Error'
...
def calc_ratio_hr(obj_info, dt, hr, cycle_huanbi, cycle_tongbi):
	...
	try:
		...
		checker_update_abs_sql ="update checker set cur_cycle = %s, status = '%s'  where id = %s"
		...
		for row in checker_rows:
			...
			if row[2] in [4, 5] :#绝对值
				...
				if (row[2] == 4 and cur_cycle > row[3]) or (row[2] == 5 and cur_cycle < row[3]):
					MU.execute('monitor', checker_update_abs_sql % (cur_cycle, 'Error', row[0]))
					type_map = {4:'绝对值大于', 5:"绝对值小于"}
					alert_message = "项目%s, %s数据异常，%s %s, 超过阈值" % (obj_info['obj_name'], row[1], type_map[row[2]], row[3])
					...
					MU.execute('monitor', checker_update_abs_sql % (cur_cycle, 'Normal', row[0]))
				continue
...
if __name__ == "__main__":
	...
	#content = {'nginx': 10, 'logtailer_kafka_to_eventarchiver': 10, 'asf': 30}

	object = 'click_hourly_monitor'
	#object = 'event_archiver_dm303'
	#object = 'event_report_sdk3x_logtailer_data'
	insert_reports_data(content, object, dt, hr, True)
